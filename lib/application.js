...
class Application extends EventEmitter {
	...
	constructor(options) {
		...
		this._marshallers.set('application/json', {
			serialize(obj) {
	 ...
	 * @param {module:x2node-webservices.Authenticator} authenticator The
	 * authenticator.
	 * @returns {module:x2node-webservices~Application} This application.
	 */
	 ...
	 * @param {module:x2node-webservices.Handler} handler The handler for the
	 * endpoint.
	 * @returns {module:x2node-webservices~Application} This application.
	 */
	...
	_respond(httpRequest, httpResponse) {
		...
		try {
			...
			this._authenticateCall(call).then(
				call => {
					this._log(`authenticated actor: ${call.actor.stamp}`);
					try {
				...
				call => {
					try {
						return call.handler[method](call);
					} catch (err) {
						return Promise.reject(err);
					}
				},
	 ...
	 * @returns {Promise.<module:x2node-webservices~ServiceCall>} Promise of the
	 * authenticated call.
	 */
	_authenticateCall(call) {
		...
		return call.authenticator.authenticate(call).then(
			actor => {
	...
	_readRequestPayload(call, httpResponse) {
		...
		if (/^multipart\//i.test(contentType)) {
			...
			// find marshaller
			const marshaller = this._marshallers.get(
	 ...
	 * @param {module:x2node-webservices.Marshaller} marshaller Marshaller to use
	 * to parse the entity.
	 ...
	 *
	 * @private
	 * @param {module:x2node-webservices.Handler} handler The handler.
