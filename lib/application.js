...
const BOUNDARY_MID = Buffer.from(`--${BOUNDARY}\r\n`, 'ascii');
const BOUNDARY_END = Buffer.from(`--${BOUNDARY}--`, 'ascii');
...
/**
 * Used to store next API call id on sockets.
 *
 * @private
 * @constant {Symbol}
 */
const NEXT_CALL_ID = Symbol('NEXT_CALL_ID');

/**
 * Function to be added to sockets to get next API call id for the connection.
 *
 * @private
...
function nextCallId() {
	...
	return `${this[CONNECTION_ID]}-${this[NEXT_CALL_ID]++}`;
}
...
/**
 * Used to store the list of service calls associated with a socket.
 */
...
/**
 * Pattern used to parse media type ranges in the "Accept" header.
 */
const ACCEPT_HEADER_RANGE_PATTERN = new RegExp(
...
class Application extends EventEmitter {
	 ...
	 * @param {function} [onShutdown] Optional function called when the
	 * application starts shutting down right after the HTTP server is asked to
	 * shut down, but before it completes shutting down.
