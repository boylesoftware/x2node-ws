...
class Application extends EventEmitter {
	 ...
	 * parameters are groups in the pattern. The URI is matched against the
	 * pattern as a whole, so no starting <code>^</code> and ending
	 * <code>$</code> are necessary. The match is case-sensitive.
	...
	_readRequestPayload(call, httpResponse) {
		...
		if (/^multipart\//i.test(contentType)) {
			...
			const marshaller = this._marshallers.lookup(entityContentType);
			if (!marshaller)
				return Promise.reject(
