...
const URIMapper = require('./uri-mapper.js');
...
/**
 * Callback on the timeout on the socket before response gas started to be sent.
 *
 * @private
...
class Application extends EventEmitter {
	...
	constructor(options) {
		...
		// authenticator, authorizer and endpoint collections (later mappers)
	...
	run(port) {
		...
		// compile URI mappings
		this._authenticators = new URIMapper(this._authenticators);
		this._authorizers = new URIMapper(this._authorizers);
		this._endpoints = new URIMapper(this._endpoints);
	...
	_respond(httpRequest, httpResponse) {
		...
		try {
			...
			httpRequest.connection.setTimeout(0, onBeforeResponseTimeout);

			// install connection close listener
			httpRequest.connection.on('close', () => {
				this._log('connection closed');
				call.connectionClosed = true;
			});

			// lookup the handler
			if (!this._endpoints.lookup(call, (handler, uriParams) => {
				call.setHandler(handler, uriParams);
			}))
			...
			this._authenticators.lookup(call, authenticator => {
				call.setAuthenticator(authenticator);
			});

			// lookup the authorizer
			this._authorizers.lookupMultiReverse(call, authorizer => {
				call.addAuthorizer(authorizer);
			});
			...
			this._authenticateCall(call).then(
				call => {
					...
					try {
						return this._authorizeCall(call);
					} catch (err) {
	...
	_readRequestPayload(call, httpResponse) {
		...
		const connection = call.httpRequest.connection;
