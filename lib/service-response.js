 ...
 *
 * @typedef {Object} module:x2node-webservices~ServiceResponse~Entity
...
class ServiceResponse {
	 ...
	 * [createResponse()]{@link module:x2node-webservices.createResponse}
	 * function.
	 *
	 * @protected
	 ...
	 * @returns {module:x2node-webservices~ServiceResponse} This response object.
	 */
	 ...
	 *
	 * @param {string} name HTTP response header name, case-insensitive.
	 * @param {string} value Header(s) to add. Multiple headers can be specified
	 * in a comma-separated list. The case of the headers is not important as all
	...
	addToHeadersListHeader(name, value) {
		...
		for (let v of String(value).trim().split(/\s*,\s*/))
			values.add(
	 ...
	 *
	 * @param {string} name HTTP response header name, case-insensitive.
	 * @param {string} value Method(s) to add. Multiple methods can be specified
	 * in a comma-separated list. The case of the methods is not important as all
	 * the values are automatically upper-cased.
	 * @returns {module:x2node-webservices~ServiceResponse} This response object.
	 */
	 ...
	 * attachment, the HTTP response is send as content type "multipart/mixed".
	 ...
	 * object, as a buffer or as a readable stream (which will trigger "chuncked"
	 ...
	 * array if none.
	 *
	 * @member {Array.<module:x2node-webservices~ServiceResponse~Entity>}
	 * @readonly
	 */
	get entities() {
